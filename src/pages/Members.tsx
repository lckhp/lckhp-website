...
const Members: React.FC<MembersProps> = ({ year }) => {
  ...
  useEffect(() => {
    document.title = "Leo Club Members | Leo Club of Kathmandu Himalayas Patan";
  }, []);

  // Filter members based on search term and active filter
  useEffect(() => {
    let filtered = membersData as Member[];
      ...
      <div className="sr-only" aria-hidden="true">
        ...
        <ul>
          {membersData.map((member) => (
            <li key={member.id}>
              <Link to={`/2425/members/${member.id}`}>
  ...
  const { generalCount, proposedCount, inactiveCount } = getMemberCounts(
    membersData as Member[]
  );
    ...
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-center mx-4 flex-1">
              ...
              <p className="mt-1 text-lg text-gray-600">
                Leo Club of Kathmandu Himalayas Patan L.Y. 24/25
              </p>
            </div>
            <img
        ...
        <div className="mb-8 px-4 py-3 bg-white text-center rounded-lg shadow-sm">
          {filteredMembers.length === membersData.length ? (
            <>
              {(() => {
                const { generalCount, proposedCount, inactiveCount } =
                  getMemberCounts(membersData as Member[]);
                return (
                  <span className="text-lg text-gray-700">
                    Showing all {membersData.length} members ({" "}
